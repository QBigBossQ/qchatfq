    ...
    socket.onmessage = function(e) {
        ...
        if(data.type == 'typing') {
            ...
            if(usersTyping.length == 1) {
                string = usersTyping + ' escribiendo...';
            } else if(usersTyping.length > 4) {
                string = 'Hay muchas personas no me maten...';
            } else if(usersTyping.length > 1) {
                var lastUser = usersTyping.pop();
                string = usersTyping.join(', ') + ' y ' + lastUser + ' estan escribiendo...';
            ...
            switch(data.info) {
                case 'rejected':
                    ...
                    if(data.reason == 'length') {
                        message = 'Tu nombre tiene que ser mayor que 3 caracteres pero menor que 16';
                    }

                    if(data.reason == 'format') {
                        message = 'Solo se pueden alfanumerico tu nombre';
                    }

                    if(data.reason == 'taken') {
                        message = 'Ya existe este nombre';
                    }

                    if(data.reason == 'banned') {
                        message = 'Baniado por el admin por' + data.time / 60 / 1000 + ' minutes. You have to wait until you get unbanned to be able to connect again';
                    }
                ...
                case 'success':
                    document.getElementById('send').childNodes[0].nodeValue = 'Enviar';
                    updateBar('mdi-content-send', 'Pon tu mensaje', false);
                    connected = true;
                ...
                case 'update':
                    showChat('info', null, data.user.oldun + ' cambio su nombre por ' + data.user.un);
                    clients[data.user.id] = data.user;
                    break;

                case 'connection':
                    var userip = data.user.ip ? ' [' + data.user.ip + ']' : '';
                    showChat('info', null, data.user.un + userip + ' conectado a el servidor');

                    clients[data.user.id] = data.user;
                    document.getElementById('users').innerHTML = Object.keys(clients).length + ' USUARIOS';
                    break;
                ...
                case 'disconnection':
                    ...
                    if(data.user.un != null) {
                        showChat('info', null, data.user.un + userip + ' SE DESCONECTO DEL server');
                    }
                ...
                case 'spam':
                    showChat('global', null, 'tienes que esperar al menos 1 seg para enviar o te BANEO ' + data.warn + ' of 5');
                    break;
